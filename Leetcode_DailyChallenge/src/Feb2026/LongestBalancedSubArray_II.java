...
class SegmentTree {
	...
	/**
	 * Push down pending lazy values from node to its children This applies deferred
	 * updates before we read/modify this node
	 */
	...
	/**
	 * Add 'val' to all elements in range [l, r] node: current node index in segment
	 * tree [start, end]: range represented by current node [l, r]: range we want to
	 * update val: value to add
	 */
	...
	/**
	 * Find the leftmost index where balance == 0 Returns -1 if no such index exists
	 */
	public int getLeftMost(int node, int start, int end) {
		...
		if (left != -1)
			return left; // found in left, return immediately
...
class LongestBalancedSubArray_II {
	public int longestBalanced(int[] nums) {
		...
		// balance[i] = (count of distinct evens from i to end) - (count of distinct
		// odds from i to end)
